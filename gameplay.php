...
if(GameHasStarted($game_result) > 0){
  ...
  if(DB()->NumRows($goals) <= 0){
    ...
    while($row = DB()->FetchAssoc($home_team_score_board))	{
      $html .= "<tr>";
    ...
    while($row = DB()->FetchAssoc($guest_team_score_board)){
      $html .= "<tr>";
      $html .= "<td style='text-align:right'>". $row['num'] ."</td>";
      $html .= "<td><a href='?view=playercard&amp;series=0&amp;player=". $row['player_id'];
      $html .= "'>". utf8entities($row['firstname']) ."&nbsp;";
    ...
    if(!intval($game_result['isongoing'])){
      ...
      $goal = DB()->FetchAssoc($allgoals);
      $turnover = DB()->FetchAssoc($turnovers);
      ...
      DB()->DataSeek($allgoals, 0);

      //loop all goals
      while($goal = DB()->FetchAssoc($allgoals)){
        //halftime passed
        ...
        if(DB()->NumRows($turnovers)){
          $turnovers = GameTurnovers($gameId);
        }
        while($turnover = DB()->FetchAssoc($turnovers)){
    ...
    if(ShowDefenseStats()){
      ...
      while($row = DB()->FetchAssoc($home_team_defense_board)){
        $html .= "<tr>";
        $html .= "<td style='text-align:right'>". $row['num'] ."</td>";
        $html .= "<td><a href='?view=playercard&amp;series=0&amp;player=". $row['player_id'];
        $html .= "'>". utf8entities($row['firstname']) ."&nbsp;";
      ...
      while($row = DB()->FetchAssoc($guest_team_defense_board)){
        $html .= "<tr>";
        $html .= "<td style='text-align:right'>". $row['num'] ."</td>";
        $html .= "<td><a href='?view=playercard&amp;series=0&amp;player=". $row['player_id'];
        $html .= "'>". utf8entities($row['firstname']) ."&nbsp;";
      ...
      $prevdefense = 0;
      DB()->DataSeek($defenses, 0);
      while($defense = DB()->FetchAssoc($defenses)){
        // 		if (!$bHt && $game_result['halftime']>0 && $goal['time'] > $game_result['halftime']){
        // 			$html .= "<tr><td colspan='6' class='halftime'>"._("Half-time")."</td></tr>";
        // 			$bHt = 1;
        // 			$prevgoal = intval($game_result['halftime']);
