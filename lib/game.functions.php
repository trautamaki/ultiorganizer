...
function PoolGameSetResults($pool, $games) {
	$query = sprintf("SELECT time, k.name As hometeamname, v.name As visitorteamname, p.*,s.name AS gamename
		...
		DB()->RealEscapeString(implode(",", $games)),
		(int)$pool);
	$result = DB()->DBQuery($query);
	if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
	return $result;
...
function AddGameMediaEvent($gameId, $time, $urlId) {
	if (hasAddMediaRight()){
		$lastnum = DB()->DBQueryToValue("SELECT MAX(num) FROM uo_gameevent WHERE game=".intval($gameId));
...
function GameUpdateResult($gameId, $home, $away) {
  if (hasEditGameEventsRight($gameId)) {
    $query = sprintf(
        ...
        DB()->RealEscapeString($home), DB()->RealEscapeString($away), DB()->RealEscapeString($gameId));
    $result = DB()->DBQuery($query);
    if (!$result) {
...
function GameSetDefenses($gameId, $home, $away) {
  if (hasEditGameEventsRight($gameId)) {
    $query = sprintf("UPDATE uo_game SET homedefenses='%s', visitordefenses='%s' WHERE game_id='%s'",
        DB()->RealEscapeString($home),
        DB()->RealEscapeString($away),
        DB()->RealEscapeString($gameId));
    $result = DB()->DBQuery($query);
    if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
    if (IsFacebookEnabled()) {
...
function GameAddScore($gameId, $pass, $goal, $time, $number, $hscores, $ascores, $home, $iscallahan) {
	if (hasEditGameEventsRight($gameId)) {
		$query = sprintf("
			...
			DB()->RealEscapeString($gameId),
			DB()->RealEscapeString($number),
			DB()->RealEscapeString($pass),
			DB()->RealEscapeString($goal),
			DB()->RealEscapeString($time),
			DB()->RealEscapeString($hscores),
			DB()->RealEscapeString($ascores),
			DB()->RealEscapeString($home),
			DB()->RealEscapeString($iscallahan),
			DB()->RealEscapeString($pass),
			DB()->RealEscapeString($goal),
			DB()->RealEscapeString($time),
			DB()->RealEscapeString($hscores),
...
function GameAddDefense($gameId, $player, $home, $caught, $time, $iscallahan, $number) {
	if (hasEditGameEventsRight($gameId)) {
		$query = sprintf("
			...
			DB()->RealEscapeString($gameId),
			DB()->RealEscapeString($number),
			DB()->RealEscapeString($player),
			DB()->RealEscapeString($time),
			DB()->RealEscapeString($iscallahan),
			DB()->RealEscapeString($caught),
			DB()->RealEscapeString($home),
			DB()->RealEscapeString($player),
			DB()->RealEscapeString($time),
			DB()->RealEscapeString($iscallahan),
...
function GameAddScoreEntry($uo_goal) {
	if (hasEditGameEventsRight($uo_goal['game'])) {
		$query = sprintf("
			...
			(game, num, assist, scorer, time, homescore, visitorscore, ishomegoal, iscallahan) 
			VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			DB()->RealEscapeString($uo_goal['game']),
			DB()->RealEscapeString($uo_goal['num']),
			DB()->RealEscapeString($uo_goal['assist']),
			DB()->RealEscapeString($uo_goal['scorer']),
			DB()->RealEscapeString($uo_goal['time']),
			DB()->RealEscapeString($uo_goal['homescore']),
			DB()->RealEscapeString($uo_goal['visitorscore']),
			DB()->RealEscapeString($uo_goal['ishomegoal']),
			DB()->RealEscapeString($uo_goal['iscallahan']));
...
function GameGetSpiritPoints($gameId, $teamId) {
  $query = sprintf("SELECT * FROM uo_spirit_score WHERE game_id=%d AND team_id=%d",
  		(int)$gameId,
  		(int)$teamId);
  $scores = DB()->DBQueryToArray($query);
  $points = array();
...
function GameSetScoreSheetKeeper($gameId, $name) {
	if (hasEditGameEventsRight($gameId)) {
		...
		UPDATE uo_game 
		SET official='%s' 
		WHERE game_id='%s'", DB()->RealEscapeString($name), DB()->RealEscapeString($gameId));
		}else {
			$query = sprintf("
		UPDATE uo_game
		SET official=NULL
		WHERE game_id='%s'", DB()->RealEscapeString($gameId));
		}		
	$result = DB()->DBQuery($query);
	if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
...
function GameSetHalftime($gameId, $time) {
	if (hasEditGameEventsRight($gameId)) {
		if (isset($time)) {
			...
			UPDATE uo_game 
			SET halftime='%s' 
			WHERE game_id='%s'", DB()->RealEscapeString($time), DB()->RealEscapeString($gameId));
		}else {
			$query = sprintf("
			UPDATE uo_game 
			SET halftime=NULL 
...
function AddGame($params) {
	...
	if (hasEditGamesRight($poolinfo['series'])) {
		$query = sprintf("
			...
			VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			DB()->RealEscapeString($params['hometeam']),
			DB()->RealEscapeString($params['visitorteam']),
			DB()->RealEscapeString($params['reservation']),
			DB()->RealEscapeString($params['time']),
			DB()->RealEscapeString($params['pool']),
			DB()->RealEscapeString($params['valid']),
			DB()->RealEscapeString($params['respteam']));
