...
function SeriesResults()
{
	if (empty($_POST['searchser'])) {
		...
		foreach ($selected as $seasonid => $value) {
			$query .= "'" . GetDatabase()->RealEscapeString($seasonid) . "', ";
		}
		$query = substr($query, 0, strlen($query) - 2);
		$query .= ")";
		if (!empty($_POST['seriesname']) && strlen(trim($_POST['seriesname'])) > 0) {
			$query .= " AND ser.name like '%" . GetDatabase()->RealEscapeString(trim($_POST['seriesname'])) . "%'";
		}
...
function PoolResults()
{
	if (empty($_POST['searchpool'])) {
		...
		if (!empty($_POST['poolname']) && strlen(trim($_POST['poolname'])) > 0) {
			$query .= " AND pool.name like '%" . GetDatabase()->RealEscapeString(trim($_POST['poolname'])) . "%'";
		}
...
function TeamResults()
{
	if (empty($_POST['searchteam'])) {
		...
		if (!empty($_POST['teamname']) && strlen(trim($_POST['teamname'])) > 0) {
			$query .= " AND team.name like '%" . GetDatabase()->RealEscapeString(trim($_POST['teamname'])) . "%'";
		}
	...
	if (empty($_POST['searchuser'])) {
		...
		if (!empty($_POST['useseasons'])) {
			...
			foreach ($selected as $seasonid => $prop) {
				$criteria .= "'" . GetDatabase()->RealEscapeString($seasonid) . "', ";
			}
		...
		if (!empty($_POST['teamname'])) {
			...
			$criteria = substr($criteria, 0, strlen($criteria) - 2);
			$criteria .= ")) and name like '%" . GetDatabase()->RealEscapeString($_POST['teamname']) . "%')))";
		}
		if (!empty($_POST['username'])) {
			if (strlen($criteria) > 0) {
				$criteria .= " and ";
			}
			$criteria .= "(name like '%" . GetDatabase()->RealEscapeString($_POST['username']) . "%')";
		}
		...
		if (!empty($_POST['email'])) {
			...
			$criteria .= "(email like '%" . GetDatabase()->RealEscapeString($_POST['email']) . "%')";
		}
...
function PlayerResults()
{
	if (empty($_POST['searchplayer'])) {
		...
		if (!empty($_POST['username'])) {
			...
			$criteria .= "(firstname like '%" . GetDatabase()->RealEscapeString($_POST['username']) . "%'";
			$criteria .= " or lastname like '%" . GetDatabase()->RealEscapeString($_POST['username']) . "%'";
			$criteria .= " or CONCAT(firstname, ' ', lastname) like '%" . GetDatabase()->RealEscapeString($_POST['username']) . "%')";
		}
...
function ReservationResults()
{
	if (empty($_POST['searchreservation']) && empty($_GET['season'])) {
		...
		if (isset($_POST['searchgroup']) && strlen($_POST['searchgroup']) > 0) {
			$query .= "AND res.reservationgroup like '%" . GetDatabase()->RealEscapeString($_POST['searchgroup']) . "%' ";
		}
		if (isset($_POST['searchfield']) && strlen($_POST['searchfield']) > 0) {
			$query .= "AND res.fieldname like '" . GetDatabase()->RealEscapeString($_POST['searchfield']) . "' ";
		}
		if (isset($_POST['searchlocation']) && strlen($_POST['searchlocation']) > 0) {
			$query .= "AND (loc.name like '%" . GetDatabase()->RealEscapeString($_POST['searchlocation']) . "%' OR ";
			$query .= "loc.address like '%" . GetDatabase()->RealEscapeString($_POST['searchlocation']) . "%') ";
		}

		if (isset($_GET['season']) && strlen($_GET['season']) > 0) {
			$query .= "AND res.season='" . GetDatabase()->RealEscapeString($_GET['season']) . "' ";
		}
	...
	if (empty($_POST['searchgame'])) {
		...
		if (isset($_POST['searchteams']) && strlen($_POST['searchteams'])) {
			foreach (explode(',', $_POST['searchteams']) as $team) {
				$query .= "AND (vj.name LIKE '%" . GetDatabase()->RealEscapeString($team) . "%' OR kj.name LIKE '%" . GetDatabase()->RealEscapeString($team) . "%') ";
			}
		}
		$result = GetDatabase()->DBQuery($query);
		if (!$result) {
