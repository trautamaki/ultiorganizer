...
function SeasonControl() {
	...
	$seasons = Seasons();
	while($season = DB()->FetchAssoc($seasons)){
...
function SeriesResults() {
	if (empty($_POST['searchser'])) {
		...
		foreach ($selected as $seasonid => $value) {
			$query .= "'".DB()->RealEscapeString($seasonid)."', ";
		}
		$query = substr($query, 0, strlen($query) - 2);
		$query .= ")";
		if (!empty($_POST['seriesname']) && strlen(trim($_POST['seriesname'])) > 0) {
			$query .= " AND ser.name like '%".DB()->RealEscapeString(trim($_POST['seriesname']))."%'";
		} 
...
function PoolResults() {
	if (empty($_POST['searchpool'])) {
		...
		if (!empty($_POST['poolname']) && strlen(trim($_POST['poolname'])) > 0) {
			$query .= " AND pool.name like '%".DB()->RealEscapeString(trim($_POST['poolname']))."%'";
		} 
...
function TeamResults() {
	if (empty($_POST['searchteam'])) {
		...
		if (!empty($_POST['teamname']) && strlen(trim($_POST['teamname'])) > 0) {
			$query .= " AND team.name like '%".DB()->RealEscapeString(trim($_POST['teamname']))."%'";
		} 
	...
	if (empty($_POST['searchuser'])) {
		...
		if (!empty($_POST['useseasons'])) {
			...
			foreach ($selected as $seasonid => $prop) {
				$criteria .= "'".DB()->RealEscapeString($seasonid)."', ";
			}
		...
		if (!empty($_POST['teamname'])) {
			...
			$criteria = substr($criteria, 0, strlen($criteria) - 2);
			$criteria .= ")) and name like '%".DB()->RealEscapeString($_POST['teamname'])."%')))";
		}
		if (!empty($_POST['username'])) {
			if (strlen($criteria) > 0) {
				$criteria .= " and ";
			}
			$criteria .= "(name like '%".DB()->RealEscapeString($_POST['username'])."%')";
		}
		...
		if (!empty($_POST['email'])) {
			...
			$criteria .= "(email like '%".DB()->RealEscapeString($_POST['email'])."%')";
		}
...
function PlayerResults() {
	if (empty($_POST['searchplayer'])) {
		...
		if (!empty($_POST['username'])) {
			...
			$criteria .= "(firstname like '%".DB()->RealEscapeString($_POST['username'])."%'";
			$criteria .= " or lastname like '%".DB()->RealEscapeString($_POST['username'])."%'";
			$criteria .= " or CONCAT(firstname, ' ', lastname) like '%".DB()->RealEscapeString($_POST['username'])."%')";
		}
...
function ReservationResults() {
	if (empty($_POST['searchreservation']) && empty($_GET['season'])) {
		...
		if (isset($_POST['searchgroup']) && strlen($_POST['searchgroup']) > 0) {
			$query .= "AND res.reservationgroup like '%".DB()->RealEscapeString($_POST['searchgroup'])."%' ";
		}
		if (isset($_POST['searchfield']) && strlen($_POST['searchfield']) > 0) {
			$query .= "AND res.fieldname like '".DB()->RealEscapeString($_POST['searchfield'])."' ";
		}
		if (isset($_POST['searchlocation']) && strlen($_POST['searchlocation']) > 0) {
			$query .= "AND (loc.name like '%".DB()->RealEscapeString($_POST['searchlocation'])."%' OR ";
			$query .= "loc.address like '%".DB()->RealEscapeString($_POST['searchlocation'])."%') ";
		}
		
		if (isset($_GET['season']) && strlen($_GET['season']) > 0) {
			$query .= "AND res.season='".DB()->RealEscapeString($_GET['season'])."' ";
		}
	...
	if (empty($_POST['searchgame'])) {
		...
		if (isset($_POST['searchteams']) && strlen($_POST['searchteams'])) {
			foreach (explode(',',$_POST['searchteams']) as $team) {
				$query .= "AND (vj.name LIKE '%".DB()->RealEscapeString($team)."%' OR kj.name LIKE '%".DB()->RealEscapeString($team)."%') ";
			}
		}
		$result = DB()->DBQuery($query);
		if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
