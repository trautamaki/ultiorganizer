...
function LogEvent($event){
		...
		DB()->RealEscapeString($event['user_id']),
		DB()->RealEscapeString($event['ip']),
		DB()->RealEscapeString($event['category']),
		DB()->RealEscapeString($event['type']),
		DB()->RealEscapeString($event['source']),
		DB()->RealEscapeString($event['id1']),
		DB()->RealEscapeString($event['id2']),
		DB()->RealEscapeString($event['description']));
...
function EventList($categoryfilter, $userfilter){
	if(isSuperAdmin()){
		...
		foreach($categoryfilter as $cat){
			...
			$query .= sprintf("category='%s'", DB()->RealEscapeString($cat));
			$i++;
			if($i==count($categoryfilter)){	$query .= ")";}
		}
		
		if(!empty($userfilter)){
			$query .= sprintf("AND user_id='%s'", DB()->RealEscapeString($userfilter));
		}
...
function ClearEventList($ids){
	if(isSuperAdmin()){
		$query = sprintf("DELETE FROM uo_event_log WHERE event_id IN (%s)", DB()->RealEscapeString($ids));
...
function GetLastGameUpdateEntry($gameId, $source) {
	$query = sprintf("SELECT * FROM uo_event_log WHERE id1=%d AND source='%s' ORDER BY TIME DESC",
		(int)$gameId, DB()->RealEscapeString($source));	
	$result = DB()->DBQuery($query);
	if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
...
function LogPageLoad($page){
  ...
  $query=sprintf("SELECT loads FROM uo_pageload_counter WHERE page='%s'",
		DB()->RealEscapeString($page));
  $loads = DB()->DBQueryToValue($query);
  
  if($loads<0){
    $query=sprintf("INSERT INTO uo_pageload_counter (page, loads) VALUES ('%s',%d)",
        DB()->RealEscapeString($page),1);
    DB()->DBQuery($query);
...
function LogVisitor($ip){
  ...
  $query=sprintf("SELECT visits FROM uo_visitor_counter WHERE ip='%s'",
		DB()->RealEscapeString($ip));
  $visits = DB()->DBQueryToValue($query);
 
  if($visits<0){
    $query=sprintf("INSERT INTO uo_visitor_counter (ip, visits) VALUES ('%s',%d)",
        DB()->RealEscapeString($ip),1);
    DB()->DBQuery($query);
  }else{
    $visits++;
