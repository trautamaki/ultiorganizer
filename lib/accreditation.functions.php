...
function ExternalLicenseValidityList() {
	return DB()->DBQueryToArray("SELECT DISTINCT external_validity FROM uo_license WHERE external_validity IS NOT NULL AND external_validity > 0");
}

function ExternalLicenseTypes() {
	return DB()->DBQueryToArray("SELECT DISTINCT external_type FROM uo_license WHERE external_type IS NOT NULL AND external_type > 0");
}

function LicenseData($accreditation_id) {
	return DB()->DBQueryToRow("SELECT membership, license, external_id, external_type, external_validity, ultimate 
		FROM uo_license WHERE accreditation_id='".DB()->RealEscapeString($accreditation_id)."'");
}
	...
	if ($userid = DB()->FetchRow($result)) {
		...
		if (validEmail($playerInfo['email'])) {
			$query = sprintf("SELECT userid FROM uo_users  WHERE LOWER(email)='%s'",
				DB()->RealEscapeString(strtolower($playerInfo['email'])));
			$result = DB()->DBQuery($query);
			if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
			if ($userId = DB()->FetchRow($result)) {
				$id = $userId[0];
...
function AccreditationLogEntry($player, $team, $source, $value, $game = NULL) {
	...
	$query = sprintf("INSERT INTO uo_accreditationlog (player, team, userid, source, value, time, game) VALUES (%d, %d, '%s', '%s', %d, now(), %s)",
		(int)$player, 
		(int)$team, 
		DB()->RealEscapeString($_SESSION['uid']), 
		DB()->RealEscapeString($source), 
		(int)$value,
		$gameVal);
	$result = DB()->DBQuery($query);
	if (!$result) { die('Invalid query: ' . DB()->SQLError()); }
