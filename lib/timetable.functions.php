...
function TimetableGames($id, $gamefilter, $timefilter, $order, $groupfilter=""){
  ...
  {
    case "season":
      $query .= " WHERE pp.valid=true AND ps.season='".DB()->RealEscapeString($id)."'";
      break;
    ...
    case "poolgroup":
      ...
      $pools = explode(",", DB()->RealEscapeString($id));
  ...
  switch($timefilter)
  {
    ...
    default:
      $query .= " AND DATE_FORMAT(pp.time,'%Y-%m-%d') = '".DB()->RealEscapeString($timefilter)."'";
      break;
  }

  if(!empty($groupfilter) && $groupfilter!="all"){
    $query .= "AND pr.reservationgroup='".DB()->RealEscapeString($groupfilter)."'";
  }
...
function TimetableFields($reservationgroup, $season){
  ...
  $query .= " WHERE pp.valid=true AND ps.season='".DB()->RealEscapeString($season)."' AND pr.reservationgroup='".DB()->RealEscapeString($reservationgroup)."'";
