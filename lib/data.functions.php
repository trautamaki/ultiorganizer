...
class EventDataXMLHandler
{
  ...
  function EventToXML($eventId)
  {
    ...
    if (isSeasonAdmin($eventId)) {
      ...
      //uo_season
      $seasons = $database->DBQuery("SELECT * FROM uo_season WHERE season_id='" . $database->RealEscapeString($eventId) . "'");
      $row = $database->FetchAssoc($seasons);
      $ret .= $this->RowToXML("uo_season", $row, false);

      //uo_reservation
      $reservations = $database->DBQuery("SELECT * FROM uo_reservation WHERE season='" . $database->RealEscapeString($eventId) . "'");
      while ($reservation = $database->FetchAssoc($reservations)) {
        $ret .= $this->RowToXML("uo_reservation", $reservation);
      }

      //uo_movingtime
      $times = $database->DBQuery("SELECT * FROM uo_movingtime WHERE season='" . $database->RealEscapeString($eventId) . "'");
      while ($time = $database->FetchAssoc($times)) {
        $ret .= $this->RowToXML("uo_movingtime", $time);
      }

      //uo_series
      $series = $database->DBQuery("SELECT * FROM uo_series WHERE season='" . $database->RealEscapeString($eventId) . "'");
      while ($ser = $database->FetchAssoc($series)) {
        $ret .= $this->RowToXML("uo_series", $ser, false);

        $seriesId = (int)$ser['series_id'];
        //uo_team
        $teams = $database->DBQuery("SELECT * FROM uo_team WHERE series='$seriesId'");
        while ($team = $database->FetchAssoc($teams)) {
          $ret .= $this->RowToXML("uo_team", $team, false);
          //uo_player
          $players = $database->DBQuery("SELECT * FROM uo_player WHERE team='" . $database->RealEscapeString($team['team_id']) . "'");
        ...
        $schedulings = $database->DBQuery("SELECT sched.* FROM uo_scheduling_name sched 
        ...
        while ($row = $database->FetchAssoc($schedulings)) {
          $ret .= $this->RowToXML("uo_scheduling_name", $row);
        }

        //uo_pool
        $pools = $database->DBQuery("SELECT * FROM uo_pool WHERE series='$seriesId'");
        while ($row = $database->FetchAssoc($pools)) {
          $ret .= $this->RowToXML("uo_pool", $row, false);

          //uo_team_pool
          $teampools = $database->DBQuery("SELECT * FROM uo_team_pool WHERE pool='" . $database->RealEscapeString($row['pool_id']) . "'");
          while ($teampool = $database->FetchAssoc($teampools)) {
            $ret .= $this->RowToXML("uo_team_pool", $teampool);
          }

          //uo_game
          $games = $database->DBQuery("SELECT * FROM uo_game WHERE pool='" . $database->RealEscapeString($row['pool_id']) . "'");
          while ($row = $database->FetchAssoc($games)) {
            $ret .= $this->RowToXML("uo_game", $row, false);

            //uo_goal
            $goals = $database->DBQuery("SELECT * FROM uo_goal WHERE game='" . $database->RealEscapeString($row['game_id']) . "'");
            while ($goal = $database->FetchAssoc($goals)) {
              $ret .= $this->RowToXML("uo_goal", $goal);
            }
            //uo_gameevent
            $gameevents = $database->DBQuery("SELECT * FROM uo_gameevent WHERE game='" . $database->RealEscapeString($row['game_id']) . "'");
            while ($gameevent = $database->FetchAssoc($gameevents)) {
              $ret .= $this->RowToXML("uo_gameevent", $gameevent);
            }
            //uo_played
            $playedplayers = $database->DBQuery("SELECT * FROM uo_played WHERE game='" . $database->RealEscapeString($row['game_id']) . "'");
            while ($playedplayer = $database->FetchAssoc($playedplayers)) {
        ...
        //uo_moveteams
        $moveteams = $database->DBQuery("SELECT m.* FROM uo_moveteams m
				LEFT JOIN uo_pool p ON(m.frompool=p.pool_id) 
				WHERE p.series='$seriesId'");
        while ($moveteam = $database->FetchAssoc($moveteams)) {
          $ret .= $this->RowToXML("uo_moveteams", $moveteam);
        }

        //uo_game_pool
        $gamepools = $database->DBQuery("SELECT g.* FROM uo_game_pool g
				LEFT JOIN uo_pool p ON(g.pool=p.pool_id)
				WHERE p.series='$seriesId'");
        while ($gamepool = $database->FetchAssoc($gamepools)) {
  ...
  function InsertToDatabase($name, $row)
  {
    ...
    switch ($name) {
      case "uo_season":
        ...
        $query .= ") VALUES (";
        $query .= "'" . $database->RealEscapeString($newId) . "',";
        $query .= $values;
        $query .= ")";
        $database->DBQueryInsert($query);

        AddEditSeason($database, $_SESSION['uid'], $newId);
        AddUserRole($database, $_SESSION['uid'], 'seasonadmin:' . $newId);
        break;
    ...
    foreach ($row as $key => $value) {
      if ($columns[strtolower($key)] === 'int') {
        if ($value === "NULL") {
          ...
          $values .= "'" . $database->RealEscapeString($value) . "',";
        else
