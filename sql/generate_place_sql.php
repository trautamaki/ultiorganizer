...
while ($line = fgets($h)) {
	...
	if (!isset($locations[$props[1]])) {
		echo "	runQuery($database, \"insert into pelik_location (id, name, fields, indoor, address, lat, lng) values (";
		echo $props[1] . ", " . $props[3] . ", " . $props[2] . ", " . $props[4] . ", " . $props[6] . ", " . $props[7] . ", " . trim($props[8]) . ")\");\n";
		$locations[$props[1]] = $props[3];
	}
	echo "	runQuery($database, \"insert into pelik_reservation (id, location, fieldname, reservationgroup, starttime, endtime) ";
