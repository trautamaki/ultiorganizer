...
function upgrade47()
{
	...
	while ($row = GetDatabase()->FetchAssoc($results)) {
	...
	if (!hasColumn('uo_player_profile', 'firstname')) {
		...
		while ($row = GetDatabase()->FetchAssoc($results)) {
			$query = sprintf(
				...
				GetDatabase()->RealEscapeString(trim($row['firstname'])),
				$row['accreditation_id']
			);
			runQuery($query);
		}
	...
	if (!hasColumn('uo_player_profile', 'lastname')) {
		...
		while ($row = GetDatabase()->FetchAssoc($results)) {
			$query = sprintf(
				...
				GetDatabase()->RealEscapeString(trim($row['lastname'])),
				$row['accreditation_id']
			);
			runQuery($query);
		}
...
function upgrade60()
{
	...
	while ($profile = GetDatabase()->FetchAssoc($dprofiles)) {
	...
	while ($license = GetDatabase()->FetchAssoc($licenses)) {

		$hasprofile = runQuery("SELECT * FROM uo_player_profile WHERE accreditation_id='" . $license['accreditation_id'] . "'");

		if (GetDatabase()->NumRows($hasprofile) == 0) {
			$query = sprintf(
				"INSERT INTO uo_player_profile (profile_id,firstname,lastname,birthdate,accreditation_id) VALUES
				('%s','%s','%s','%s','%s')",
				GetDatabase()->RealEscapeString($license['accreditation_id']),
				GetDatabase()->RealEscapeString($license['firstname']),
				GetDatabase()->RealEscapeString($license['lastname']),
				GetDatabase()->RealEscapeString($license['birthdate']),
				GetDatabase()->RealEscapeString($license['accreditation_id'])
			);
...
function upgrade75()
{
	if (!hasTable("uo_spirit_category")) {
		...
		$categories = array();
		while ($cat = GetDatabase()->FetchAssoc($categoriesResult)) {
	...
	if (!hasTable("uo_translation")) {
      ...
      PRIMARY KEY (`translation_key`, `locale`)
		...
		foreach ($locales as $localestr => $localename) {
			$loc = GetDatabase()->RealEscapeString(str_replace(".", "_", $localestr));
			runQuery(sprintf(
...
function hasTable($table)
{
	...
	$tables = GetDatabase()->DBQuery($query);
	while (list($temp) = GetDatabase()->FetchArray($tables)) {
		if ($temp == $table) {
			return TRUE;
		}
	}
	...
	while ($row = GetDatabase()->FetchAssoc($results)) {
		...
		if ($image) {
			...
			$query = sprintf(
				...
				GetDatabase()->RealEscapeString($imgname),
				GetDatabase()->RealEscapeString($row['club_id'])
			);
			runQuery($query);
			unlink($target);
		}
	...
	while ($row = GetDatabase()->FetchAssoc($results)) {
		...
		if ($image) {
			...
			$query = sprintf(
				...
				GetDatabase()->RealEscapeString($imgname),
				GetDatabase()->RealEscapeString($row['team_id'])
			);
			runQuery($query);
			unlink($target);
		}
	...
	while ($row = GetDatabase()->FetchAssoc($results)) {
		...
		if ($image) {
			...
			$query = sprintf(
				...
				GetDatabase()->RealEscapeString($imgname),
				GetDatabase()->RealEscapeString($row['accreditation_id'])
			);
			runQuery($query);
			unlink($target);
		}
