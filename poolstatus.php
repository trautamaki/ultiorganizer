...
function printPlayoffTree($seasoninfo, $poolinfo)
{
  ...
  foreach ($pools as $poolId) {
    ...
    for ($i = 1; $i <= $totalteams; $i++) {
      ...
      if ($team['team_id']) {
        ...
        if ($realteam['team_id'] && $frompoolinfo['played'] && GetDatabase()->NumRows($gamesleft) == 0 && !$isodd) {
      ...
      if ($team['team_id']) {
        ...
        if (GetDatabase()->NumRows($gamesinpool) == 0) { // that's the BYE team
  ...
  for ($i = 1; $i <= $totalteams; $i++) {
    ...
    if (empty($pool))
      ...
      $gamesleft = GetDatabase()->NumRows(TeamPoolGamesLeft($team['team_id'], $pool['pool_id']));
    }
    $teampart = "";
    $unknown = "";
