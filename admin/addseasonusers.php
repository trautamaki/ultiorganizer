...
if (!empty($_POST['add'])) {
  ...
  if (IsRegistered($database, $userid)) {
    if ($_GET["access"] == "eventadmin") {
      AddSeasonUserRole($database, $userid, "seasonadmin:" . $seasonId, $seasonId);
    } elseif ($_GET["access"] == "teamadmin") {
      AddSeasonUserRole($database, $userid, "teamadmin:" . $_POST["team"], $seasonId);
    } elseif ($_GET["access"] == "gameadmin") {
      $reservations = $_POST["reservations"];
      foreach ($reservations as $res) {
        $games = ReservationGames($database, $res);
        while ($game = $database->FetchAssoc($games)) {
          AddSeasonUserRole($database, $userid, 'gameadmin:' . $game['game_id'], $seasonId);
        }
      }
    } elseif ($_GET["access"] == "accradmin") {
      $teams = $_POST["teams"];
      foreach ($teams as $teamId) {
        AddSeasonUserRole($database, $userid, 'accradmin:' . $teamId, $seasonId);
      }
    }
...
$admins = SeasonAdmins($database, $seasonId);
...
$admins = SeasonTeamAdmins($database, $seasonId);
...
if (!empty($_GET["access"]) && $_GET["access"] == "teamadmin") {
  $html .= "<table style='white-space: nowrap' cellpadding='2px'>\n";
  $html .= "<tr><td>";
  $teams = SeasonTeams($database, $seasonId);
...
$seasongames = SeasonAllGames($database, $seasonId);
...
$admins = SeasonGameAdmins($database, $seasonId);
...
if (!empty($_GET["access"]) && $_GET["access"] == "gameadmin") {
  ...
  $reservations = SeasonReservations($database, $seasonId);
...
$admins = SeasonAccreditationAdmins($database, $seasonId);
foreach ($admins as $user) {
  $html .= "<tr>";
  $teaminfo = TeamInfo($database, $user['team_id']);
  $html .= "<td style='width:175px'>" . utf8entities(U_($teaminfo['seriesname'])) . ", " . utf8entities(U_($teaminfo['name'])) . "</td>\n";
