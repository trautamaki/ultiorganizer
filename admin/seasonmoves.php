...
<script type="text/javascript">
	...
	function SetPool($database, poolid, from) {
...
if (!empty($_POST['remove_x'])) {
	...
	PoolUndoMove($database, $move[0], $move[1], $move[2]);
} elseif (!empty($_POST['removeAll_x'])) {
	$params = preg_split('/:/', $_POST['hiddenPoolId']);

	if ($order == "from") { // FIXME or parms[1]?
		$moves = PoolMovingsFromPool($database, $params[0]);
	} else {
		$moves = PoolMovingsToPool($database, $params[0]);
	}

	foreach ($moves as $row) {
		if (!$row['ismoved']) {
			PoolDeleteMove($database, $row['frompool'], $row['fromplacing']);
		}
	}
	...
	for ($i = 0; $i < count($_POST['schedulingnameEdited']); $i++) {
		if ($_POST['schedulingnameEdited'][$i] == "yes") {
			...
			PoolSetSchedulingName($database, $id, $_POST["sn$i"], $season);
		}
	}
	for ($i = 0; $i < count($_POST['moveEdited']); $i++) {
		if ($_POST['moveEdited'][$i] == "yes") {
			...
			PoolSetMove($database, $frompool, $fromplacing, $newfp, $newtp);
		}
	}
}
	...
	if (count($pools)) {
		...
		foreach ($pools as $pool) {
			if ($order == "from") {
				...
				$moves = PoolMovingsToPool($database, $pool['pool_id']);
			}
			...
			if (count($moves)) {
				echo "<table class='admintable'><tr>
					...
					<th style='width:14%'><input class='button' type='submit' name='undoPool' value='" . _("Undo") . "' onclick=\"SetPool($database, " . $pool['pool_id'] . "," . ($order == "from" ? "true" : "false") . ");\"/>
						<input class='deletebutton' type='image' src='images/remove.png' alt='X' name='removeAll' value='" . _("X") . "' onclick=\"SetPool($database, " . $pool['pool_id'] . "," . ($order == "from" ? "true" : "false") . ");\"/></th></tr>";
			}
