...
if (isset($_POST['backup']) && !empty($_POST['tables']) && isSuperAdmin()) {
	...
	foreach ($tables as $table) {
		set_time_limit(120);
		$result = $database->DBQuery('SELECT * FROM ' . $table);
		$num_fields = $result->field_count;

		$return .= 'DROP TABLE IF EXISTS ' . $table . ';';
		$row2 = $database->FetchRow($database->DBQuery('SHOW CREATE TABLE ' . $table));
		$return .= "\n\n" . $row2[1] . ";\n\n";
		...
		for ($i = 0; $i < $num_fields; $i++) {
			while ($row = $result->fetch_row()) {
				...
				for ($j = 0; $j < $num_fields; $j++) {
					...
					if ($database->fieldtype($result, $j) == 'blob' && $table == 'uo_image') {
						if (isset($row[$j]) && ($row[$j] != NULL)) {
							$return .= '0x' . bin2hex($row[$j]);
						} else {
							$return .= 'NULL';
						}
					} elseif ($database->fieldtype($result, $j) == 'int') {
						if (isset($row[$j]) && ($row[$j] != NULL)) {
							$return .= intval($row[$j]);
						} else {
							$return .= 'NULL';
