...
if (isset($_POST['backup']) && !empty($_POST['tables']) && isSuperAdmin()){
	...
	foreach($tables as $table){
		set_time_limit(120);
		$result = DB()->DBQuery('SELECT * FROM '.$table);
		$num_fields = DB()->NumFields($result);
		
		$return.= 'DROP TABLE IF EXISTS '.$table.';';
		$row2 = DB()->FetchRow(DB()->DBQuery('SHOW CREATE TABLE '.$table));
		$return.= "\n\n".$row2[1].";\n\n";
		...
		for ($i = 0; $i < $num_fields; $i++){
			while($row = DB()->FetchRow($result)){
				...
				for($j=0; $j<$num_fields; $j++){
					...
					if(DB()->FieldType($result,$j)=='blob' && $table=='uo_image'){
						if (isset($row[$j]) && ($row[$j] != NULL)){ 
							$return .= '0x'.bin2hex($row[$j]);
						}else{ 
							$return.= 'NULL'; 
						}
					}elseif(DB()->FieldType($result,$j)=='int'){
						if (isset($row[$j]) && ($row[$j] != NULL)){ 
							$return .= intval($row[$j]);
						}else{ 
							$return.= 'NULL'; 
