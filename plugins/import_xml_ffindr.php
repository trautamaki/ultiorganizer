...
class XMLHandler{
    ...
    if (is_array($attribs)) {
      ...
      while(list($key,$val) = each($attribs)) {
        if($val=="NULL"){
          ...
          $row[$key]=DB()->RealEscapeString($val);
        }
      }
      switch($this->mode){
        case "new":
  ...
  function InsertToDatabase($name, $row){
    ...
    switch($name){
      ...
      case "DIVISION":
        ...
        $cond = "name='".DB()->RealEscapeString($row['NAME'])."' AND season='".$this->eventId."'";
        ...
        if(!$exist){
          ...
          $division_info=DB()->DBQueryToRow($query);
          $this->divisionId = $division_info['series_id'];
        }
        break;

      case "TEAM":
        $cond = "name='".DB()->RealEscapeString($row['NAME'])."' AND series='".$this->divisionId."'";
        ...
        if(!empty($email)){
          ...
          if(!$user){
			...
			$query = sprintf("INSERT INTO uo_userproperties (userid, name, value) VALUES ('%s', 'poolselector', 'currentseason')",
			  DB()->RealEscapeString($email));
            DB()->DBQuery($query);
            $userid=DB()->RealEscapeString($email);	  
          }else{
            $userid=$user['userid'];
          }
        }else{
          ...
          $team_info=DB()->DBQueryToRow($query);
      ...
      case "PLAYER":
        ...
        if(isset($row['FIRSTNAME'])){ 
          ...
          $cond = "firstname='".DB()->RealEscapeString($row['FIRSTNAME'])."' AND lastname='".DB()->RealEscapeString($row['LASTNAME'])."' AND team='".$this->teamId."'";
        }else{
          $row['LASTNAME']=stripslashes($row['LASTNAME']);
          $cond = "lastname='".DB()->RealEscapeString($row['LASTNAME'])."' AND team='".$this->teamId."'";
        }
        ...
        if($profileId>0){
          ...
          $query = sprintf("SELECT pp.* FROM uo_player_profile pp
								...
								DB()->RealEscapeString(strtoupper($row['LASTNAME'])));
		  if(DBQueryRowCount($query)==1){
		    $player=DB()->DBQueryToRow($query);
        ...
        if(!$exist1 && !$exist2){
          ...
          $player_info=DB()->DBQueryToRow($query);
  ...
  function SetRow($name, $row, $cond){
    ...
    $query = rtrim($query,', ');
    $query .= " WHERE ";
    $query .= DB()->RealEscapeString($cond);
    return DBQueryInsert($query);
  }
}
