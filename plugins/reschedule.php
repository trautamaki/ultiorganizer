...
if (isset($_POST['update'])) {
	...
	//$database->DBQuery("UPDATE uo_reservation SET starttime='2010-07-07 08:30:00', endtime='2010-07-07 22:30:00' WHERE reservationgroup='Day 4'");

	//re-schedule Day 4 games:
	$games = $database->DBQueryToArray("SELECT g.*, r.* FROM uo_game g 
	...
	foreach ($games as $game) {
		if (Hours($game['time']) == 8) {
			$database->DBQuery("UPDATE uo_game SET time='2010-07-07 12:15:00', timeslot=105 WHERE game_id=" . $game['game_id'] . "");
		} elseif (Hours($game['time']) == 10) {
			$database->DBQuery("UPDATE uo_game SET time='2010-07-07 14:00:00', timeslot=105 WHERE game_id=" . $game['game_id'] . "");
		} elseif (Hours($game['time']) == 13) {
			$database->DBQuery("UPDATE uo_game SET time='2010-07-07 15:45:00', timeslot=105 WHERE game_id=" . $game['game_id'] . "");
		} elseif (Hours($game['time']) == 15) {
			$database->DBQuery("UPDATE uo_game SET time='2010-07-07 17:30:00', timeslot=105 WHERE game_id=" . $game['game_id'] . "");
		}
	}
	...
	foreach ($games as $game) {
		...
		$newresid = $database->DBQueryToValue("SELECT r.id FROM uo_reservation r
						WHERE r.reservationgroup='Day 4' AND r.fieldname='" . $game['fieldname'] . "'");

		$database->DBQuery("UPDATE uo_game SET time='$timestring', timeslot=$timeslot,
