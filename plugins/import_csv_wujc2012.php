	...
	if (is_uploaded_file($_FILES['file']['tmp_name'])) {
		...
		if (($handle = fopen($_FILES['file']['tmp_name'], "r")) !== FALSE) {
			while (($data = fgetcsv($handle, 0, $separator)) !== FALSE) {
				...
				if ($country == "United Kingdom" || $country == "GREAT BRITAIN" || $country == "England" || $country == "GB") {
					$country = "Great Britain";
				}

				//if($series=="coed"){$series="mixed";}
				if ($division1 == "Open" && $division2 == "Junior") {
					$series = "Junior Open";
					$gender = "M";
				}
				if ($division1 == "Womens" && $division2 == "Junior") {
					$series = "Junior Women";
					$gender = "F";
				}
				if ($division1 == "Open" && $division2 == "Under 17") {
					$series = "U17 Open";
					$gender = "M";
				}
				if ($division1 == "Womens" && $division2 == "Under 17") {
					$series = "U17 Girls";
					$gender = "F";
				}
				...
				if ($playerId == -1) {
					...
					$query = sprintf(
						...
						GetDatabase()->RealEscapeString(strtoupper($first)),
						GetDatabase()->RealEscapeString(strtoupper($last))
					);
					$players = GetDatabase()->DBQueryToArray($query);
					if (count($players) == 0) {
