...
if (isset($_POST['save'])) {
  $query = sprintf(
    ...
    $database->RealEscapeString($_POST['lastname']),
    $database->RealEscapeString($_POST['firstname']),
    $database->RealEscapeString($_POST['membership']),
    $database->RealEscapeString($_POST['birthdate']),
    $database->RealEscapeString($_POST['accreditation_id']),
    $database->RealEscapeString($_POST['ultimate']),
    $database->RealEscapeString($_POST['women']),
    $database->RealEscapeString($_POST['junior']),
    $database->RealEscapeString($_POST['license']),
    $database->RealEscapeString($_POST['external_id']),
    $database->RealEscapeString($_POST['external_type']),
    $database->RealEscapeString($_POST['external_validity']),
    $accId
  );
  ...
  $database->DBQuery("DELETE FROM uo_license WHERE accreditation_id='" . $id . "'");
}
...
if ($accId > 0) {
  ...
  $licenses = $database->DBQuery("SELECT * FROM uo_license WHERE accreditation_id='" . $accId . "'");
  $html .= "<table>";
  $lis = $database->FetchAssoc($licenses);
  ...
  $licenses = $database->DBQuery("SELECT * FROM uo_license ORDER BY lastname");
  $html .= "<table style='width:100%'>";
  while ($lis = $database->FetchAssoc($licenses)) {
    $html .= "<tr>";
